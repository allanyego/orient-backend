...
const { sendApprovalEmail } = require("../util/email");
...
router.put("/:policyId", auth, async (req, res, next) => {
  ...
  try {
    ...
    const client = await clientCtrl.findById(updatedPolicy.client);
    ...
    if (req.body.vehicle) {
      ...
      if (isVehicle) {
        return res.json(
          createResponse({
            error:
              "There seems to a vehicle already registered with those details.",
          })
        );
      }
    }
